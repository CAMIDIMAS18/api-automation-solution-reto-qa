...
Necesidad del negocio: Consultar las reservaciones agendadas
  ...
  Regla: Los parámetros de búsqueda permitidos son: idBooking, firstname & lastname, Checkin to Checkout, All Bookings.
    ...
    @viewBookingDetails @FunctionalTest
    Escenario: [YAPE-GB-003] Validar que se pueda consultar una reserva por su BookingId
    ...
    Escenario: [YAPE-GB-004] Validar que se puedan consultar todas las reservas existentes
    ...
    Escenario: [YAPE-GB-005] Validar que se pueda consultar una reserva por los nombres del cliente
      Cuando ella filtre la consulta por el parámetro "Customer Names"
      Entonces visualizará el Id de su reserva

    @getBookingByDates @FunctionalTest @failedTest
    Escenario: [YAPE-GB-006] Validar que se pueda consultar una reserva por un rango de fechas especificas
    ...
    Esquema del escenario: [YAPE-GB-007] Ver todos los IDs de las reservas consultando por fechas especificas
      Cuando ella filtre la consulta por las fechas entre "<checkin>" a "<checkout>"
      Entonces visualizará todos los bookingIDs encontrados para el rango de fechas
      Ejemplos:
        | checkin    | checkout   |
        | 2010-01-01 | 2022-01-23 |